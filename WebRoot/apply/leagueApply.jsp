		...
		<script type="text/javascript">
			...
			function handle(uid){
				$.actions({
					actions: [{
							text: "同意",
							onClick: function() {
								$.confirm("同意其加入裁判组？", function() {
								  	//点击确认后的回调函数
								  	$.post("${pageContext.request.contextPath}/apply/agreeLeagueApply",{"uid":uid,"lid":lid},function(data){
							...
							onClick: function() {
								$.prompt("请说明拒绝理由", function(text) {
								  ...
								  if(text.trim().length > 20){
								  	...
								  	$.post("${pageContext.request.contextPath}/apply/rejectLeagueApply",{"uid":uid,"lid":lid,"reason":text.trim()},function(data){
										var json = $.parseJSON(data);
						        		if(json.data) {
						        			//获取头像链接
										  	var imgSrc = $("#img_"+uid)[0].src; 
						...
						},
						{
							...
							onClick: function() {
								$.confirm("确认忽略？", function() {
								  	//点击确认后的回调函数
								  	$.post("${pageContext.request.contextPath}/apply/ignoreLeagueApply",{"uid":uid,"lid":lid},function(data){
										var json = $.parseJSON(data);
						        		if(json.data) {
						        			//获取头像链接
										  	var imgSrc = $("#img_"+uid)[0].src; 
			...
			$(function(){
				...
				$(document.body).infinite().on("infinite", function() {
				  ...
				  if(falg){
					//请求数据
					  $.post("${pageContext.request.contextPath}/apply/ajaxLoadMoreApplyHandleList",{"pageNo":page,"lid":lid},function(data){
						  //请求到数据
		...
		<div class="weui-tab__bd">
				...
				<div class="weui-panel weui-panel_access">
					<div align="center" id="wait">
						<h2 class="weui-panel__hd titleclick" hide="1"><font color="#09BB07" size="3px">待处理列表(<span id="waitHandleNum"><s:property value="#waitLeagueHandleList.size()"/></span>)&nbsp;&nbsp;&nbsp;&nbsp;</font>
						<img id="img1" src="${pageContext.request.contextPath}/common/img/bottom.png" style="width:15px;hight:15px;margin-bottom: -2px" >
						</h2>
					</div>
					
					<div class="weui-panel__bd" id="waitHandle">
						<s:if test="#waitLeagueHandleList != null && #waitLeagueHandleList.size()>0">
			    			<s:iterator value="#waitLeagueHandleList" status="status" var="waitHandle">
									...
									<div class="weui-media-box__hd" style="padding:10px auto">
									    ...
									    <s:else> 
									    	<s:if test="#waitHandle.headPortrait == null || #waitHandle.headPortrait == ''"> 
									 		...
									 		<s:else>
									    		<img id="img_${waitHandle.id}" style="border-radius:50%;" height="70px" width="70px" class="weui-media-box__thumb" src="${pageContext.request.contextPath}/image/${waitHandle.headPortraitNew}" alt="头像">
									 		</s:else>  
									    </s:else>
									</div>
				...
				<div class="weui-panel weui-panel_access">
					<div align="center" id="handle">
						<h2 class="weui-panel__hd titleclick" hide="1"><font color="#09BB07" size="3px">已处理列表(<span id="handleNum"><s:property value="#tempLeagueHandleList.size()"/></span>)&nbsp;&nbsp;&nbsp;&nbsp;</font>
						   <img id="img2" src="${pageContext.request.contextPath}/common/img/bottom.png" style="width:15px;hight:15px;margin-bottom: -2px" >
						</h2>
					</div>
					
					<div id="handleDiv" class="weui-panel__bd">
						<!-- 已处理列表 -->
						<s:if test="#leagueHandleList != null && #leagueHandleList.size()>0">
			    			<s:iterator  value="#leagueHandleList" status="status" var="handle">
