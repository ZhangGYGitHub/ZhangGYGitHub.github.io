...
{
    var defaults = { // 默认值。 
        title:'请选择新增类型',       // 标题文本，若不想显示title请通过CSS设置其display为none 
        showTitle:true,     // 是否显示标题栏。
        closeText:'[取消]', // 关闭按钮文字，若不想显示关闭按钮请通过CSS设置其display为none 
    ...
    var barHtml = !options.showTitle ? '' :
        '<div class="bar"><span class="title"><h1>' + options.title + '</h1></span>'+
        '<div class="login_pop_t"> <a id="hide" class="jqmClose">'
        +'<img src="../App_Themes/Tehemes/Image/pop/closeUnselectPic.gif"'
        +' onmouseover="this.src=\'../App_Themes/Tehemes/Image/pop/closeSelect.gif\'"'
        +' onmouseout="this.src=\'../App_Themes/Tehemes/Image/pop/closeUnselectPic.gif\'" /></a></div>'
        +'</div>';
    var dialog = $('<div id="' + options.id + '" class="dialog">'
    		+barHtml+'<div class="contentdia">'
    		+' <div class="login_foot">  <a class="close" onclick="setleixing(0)" >部门</a></div>'
    		+'<div class="login_foot">  <a class="close" onclick="setleixing(1)" >子公司</a></div>'
    		+'</div></div>').hide();
    $('body').append(dialog);
    ...
    var resetPos = function()
    {
        ...
        if(options.center)
        {
            ...
            var top = ($(window).height() - dialog.height()) / 2-70;
    ...
    var init = function()
    {
        ...
        dialog.find('#hide').bind('click', this.hide);
